...
// Classes: Used to define a blueprint for objects.
class student implements studentType{
    name: string
    courseEnrolled: string
    readonly fee: number // To make class properties immutable, you can use the readonly keyword that control access to the properties.
...
to initialize the object's properties with the values you pass in (arguments). */
    constructor(n: string, c: string, f: number){ 
        this.name = n
